...
public func runFlowmoChecks() -> Int32 {
    ...
    do {
        ...
        Check.expectNear(
            engine.status(now: t0.addingTimeInterval(900)).remaining ?? -1, 90, "break remaining after continue")
    } catch {
    ...
    do {
        ...
        Check.expect(engine.world.history.isEmpty, "close beat waits for dismissal before recording")
        ...
        Check.expect(engine.world.history.count == 1, "dismiss records completion once")
        Check.expectNear(engine.world.history[0].focusSeconds, 600, "dismiss records focus")
        do {
            try engine.apply(.skip, now: t0.addingTimeInterval(50_002))
            Check.expect(false, "second dismissal should fail")
        } catch let error as EngineError {
            Check.expect(error == .nothingRunning, "second dismissal is nothingRunning")
        }
        Check.expect(engine.world.history.count == 1, "failed second dismissal does not double-record")
    } catch {
    ...
    do {
        ...
        Check.expectEqual(
            engine.world.live?.focusStartedAt, t0.addingTimeInterval(120), "late prime preserves deadline")
        Check.expectNear(
            engine.status(now: t0.addingTimeInterval(180)).elapsed, 60, "late prime preserves focus elapsed")
    } catch {
    ...
    do {
        ...
        Check.expectEqual(
            engine.world.live?.phaseStartedAt, t0.addingTimeInterval(900), "catch-up close beat uses recall deadline")
        Check.expect(engine.world.history.isEmpty, "catch-up waits for close-beat dismissal")
        engine.sync(now: t0.addingTimeInterval(2_000))
        Check.expect(engine.world.history.isEmpty, "later sync still waits for dismissal")
        try engine.apply(.skip, now: t0.addingTimeInterval(2_001))
        Check.expect(engine.world.history.count == 1, "catch-up completion records on dismissal")
    } catch {
    ...
    do {
        ...
        try engine.apply(.pauseForRecovery, now: t0.addingTimeInterval(700))
        try engine.apply(.restart, now: t0.addingTimeInterval(800))
        Check.expect(engine.world.live?.phase == .prime, "close-beat restart begins prime")
        Check.expect(engine.world.history.isEmpty, "close-beat restart discards completion")
        Check.expectEqual(engine.world.profile, originalProfile, "close-beat restart does not adapt profile")
    } catch {
        Check.expect(false, "close-beat restart threw \(error)")
    }

    do {
        var engine = try loopToCloseBeat()
        let originalProfile = engine.world.profile
        try engine.apply(.cancel, now: t0.addingTimeInterval(604))
        Check.expect(engine.world.live == nil, "close-beat cancel returns idle")
        Check.expect(engine.world.history.isEmpty, "close-beat cancel discards completion")
        Check.expectEqual(engine.world.profile, originalProfile, "close-beat cancel does not adapt profile")
    } catch {
        Check.expect(false, "close-beat cancel threw \(error)")
    }
    ...
    do {
        ...
        Check.expectEqual(
            engine.world.live?.phaseStartedAt, t0.addingTimeInterval(720), "stale skip keeps automatic boundary")
        Check.expectNear(
            engine.status(now: t0.addingTimeInterval(721)).remaining ?? -1, 179, "stale skip leaves recall running")
    ...
    Check.expectEqual(
        FocusGuard.normalize([
            "  com.apple.Safari ", "com.apple.Safari", "app.flowmo.mac", "", "com.apple.finder", "com.apple.Mail",
        ]),
    ...
    do {
        var engine = Engine()
        try engine.apply(
            .configureFocusGuard(FocusGuardConfiguration(enabled: true, bundleIdentifiers: ["com.apple.Safari"])),
            now: t0)
        ...
        do {
            try engine.apply(
                .configureFocusGuard(FocusGuardConfiguration(enabled: false, bundleIdentifiers: [])), now: t0)
    ...
    do {
        ...
        let sid = UUID()
        let safari = FocusGuardProcessIdentity(
            processIdentifier: 42,
            bundleIdentifier: "com.apple.Safari",
            launchDate: t0
        )
        let reusedSafariPID = FocusGuardProcessIdentity(
            processIdentifier: 42,
            bundleIdentifier: "com.apple.Safari",
            launchDate: t0.addingTimeInterval(1)
        )
        let mail = FocusGuardProcessIdentity(
            processIdentifier: 1,
        ...
        Check.expectEqual(
            runtime.activated(processIdentity: mail, displayName: "Mail", isSelf: false),
            .ignore,
        ...
        Check.expectEqual(
            runtime.activated(processIdentity: safari, displayName: "Safari", isSelf: false),
            .allowedOnce,
            "same process instance allowed once"
        )
        Check.expectEqual(
            runtime.activated(processIdentity: reusedSafariPID, displayName: "Safari", isSelf: false),
            .intercept,
            "PID-reused process does not inherit allow once"
        )
        runtime.stayFocused()
        runtime.deactivated(processIdentity: reusedSafariPID)
        Check.expectEqual(
            runtime.activated(processIdentity: safari, displayName: "Safari", isSelf: false),
            .allowedOnce,
            "different instance deactivation does not clear allow once"
        )
        runtime.deactivated(processIdentity: safari)
        Check.expectEqual(
            runtime.activated(processIdentity: safari, displayName: "Safari", isSelf: false),
            .intercept,
            "exact process deactivation ends allow once"
        )
    ...
    do {
        ...
        let firstDemand = FocusGuardDemand.active(
            sessionID: firstSession,
            bundleIdentifiers: ["com.apple.Safari"]
        )
        let secondDemand = FocusGuardDemand.active(
            sessionID: secondSession,
            bundleIdentifiers: ["com.apple.Safari"]
        )

        runtime.setDemand(firstDemand)
        let stableGeneration = runtime.demandGeneration
        runtime.setDemand(firstDemand)
        Check.expectEqual(
            runtime.demandGeneration,
            stableGeneration,
            "unchanged active demand keeps its generation"
        )
        _ = runtime.activated(processIdentity: safari, displayName: "Safari", isSelf: false)
        let firstWork = runtime.currentWork
        Check.expect(firstWork.map(runtime.isCurrent) == true, "current guard work accepted")

        runtime.setDemand(secondDemand)
        Check.expect(firstWork.map(runtime.isCurrent) == false, "session change rejects stale guard work")
        _ = runtime.activated(processIdentity: safari, displayName: "Safari", isSelf: false)
        let beforePause = runtime.currentWork
        let beforePauseGeneration = runtime.demandGeneration
        runtime.setDemand(.inactive)
        Check.expect(
            beforePause.map(runtime.isCurrent) == false,
            "inactive demand rejects pending guard work"
        )
        runtime.setDemand(secondDemand)
        Check.expect(
            runtime.demandGeneration > beforePauseGeneration,
            "inactive-active transition advances demand generation"
        )
        Check.expect(
            beforePause.map(runtime.isCurrent) == false,
            "pause-continue rejects work from the prior active generation"
        )

        _ = runtime.activated(processIdentity: safari, displayName: "Safari", isSelf: false)
        let beforeSelectionChange = runtime.currentWork
        runtime.setDemand(
            .active(
                sessionID: secondSession,
                bundleIdentifiers: ["com.apple.Safari", "com.apple.Mail"]
            ))
        Check.expect(
            beforeSelectionChange.map(runtime.isCurrent) == false,
            "selected-app change rejects stale guard work"
        )
    }
    ...
    do {
        ...
        Check.expectEqual(
            Format.glance(engine.status(now: t0.addingTimeInterval(200))), "· 01:05", "paused glance frozen")
    } catch {
    ...
    do {
        ...
        try engine.apply(.start(intention: "x"), now: t0)
        Check.expectNear(
            TimedNotice.remainingToSchedule(engine.status(now: t0)) ?? -1, 120, "prime schedules remaining")
        ...
        Check.expectNear(
            TimedNotice.remainingToSchedule(engine.status(now: t0.addingTimeInterval(50))) ?? -1, 10,
            "break schedules remaining")
        try engine.apply(.skip, now: t0.addingTimeInterval(51))
        Check.expectNear(
            TimedNotice.remainingToSchedule(engine.status(now: t0.addingTimeInterval(51))) ?? -1, 180,
            "skip break schedules reflection")
    } catch {
